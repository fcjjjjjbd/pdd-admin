...
import { hideLoading, showLoading, showToast } from '../../utils/common';
import { MERCHANT_STATE } from '../../utils/config';
...
const state = computed(() => unref(formData).state != undefined);
const stateText = computed(() => MERCHANT_STATE[unref(formData).state] || '提交审核');
const stateBtnStyle = computed(() => {
	const state = unref(formData).state;
	return [0, 2].includes(state) ? 'warn' : 'primary';
});
const formRef = ref(null);
const areaValue = ref(null);
const toolsCloudObj = uniCloud.importObject('public-tools', {
	customUI: true
});
...
const submitForm = async () => {
	try {
		...
		let { errCode } = await merchantCloudObj.add(unref(formData));
		if (errCode !== 0) return showToast('申请失败请重试');
		showToast('申请成功，等待审核');
		getDetail();
	} catch (err) {
		showToast(err);
...
<template>
	<view class="page-wrap">
		...
		<view class="form-wrap" :class="{ 'form-disabled': state }">
			<uni-forms ref="formRef" :modelValue="formData" :rules="formRules" label-width="80px" label-align="right">
				...
				<view class="form-item-wrap">
					<uni-forms-item label="手机号" name="phone" required>
						<view class="input-wrap">
							<uni-easyinput class="input disabled-input-click" placeholder="清输入手机号" v-model.trim="formData.phone" disabled />
				...
				<view class="form-item-wrap">
					<uni-forms-item label="合作地区" name="area" required>
						<uni-data-picker
							class="input"
							placeholder="请选择地址"
							popup-title="请选择城市"
			...
			<view class="submit-wrap">
				<button :type="stateBtnStyle" class="submit" :disabled="state" @click.stop="submitForm">{{ stateText }}</button>
			</view>
		</view>

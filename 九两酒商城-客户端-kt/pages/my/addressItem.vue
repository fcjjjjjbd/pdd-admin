...
import { hideLoading, showLoading, showModal, showToast } from '../../utils/common';
const id = ref(null);
...
const handleRemove = async () => {
	try {
		...
		let { errCode, data } = await addressCloudObj.remove({ _id: unref(formData)._id });
...
const submitForm = async () => {
	try {
		...
		showLoading({ mask: true });
		let { errCode } = unref(formData)._id ? await addressCloudObj.update(unref(formData)) : await addressCloudObj.add(unref(formData));
	...
	<view class="page-wrap">
		<view class="form-wrap">
			<uni-forms ref="formRef" :modelValue="formData" :rules="formRules" label-width="80px" label-align="right">
				...
				<view class="form-item-wrap">
					<uni-forms-item label="联系电话" name="phone" required>
						<view class="input-wrap">
							<uni-easyinput class="input disabled-input-click" placeholder="清输入联系电话" v-model.number="formData.phone" />
						</view>
					</uni-forms-item>
				</view>
			...
			<view class="submit-wrap">
				<button class="btn" v-if="formData._id" type="warn" plain @click.stop="handleRemove">删除收货地址</button>

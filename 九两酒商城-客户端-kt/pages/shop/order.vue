...
const { transportType, transportRef, transportList, transportSelect, handleTransport, getTransportFee } = useTransport();
const { payRef, paySuccess, payCancel, payFail, payCreate } = usePay(deliveryType);
...
locationStore.doLocation().finally(async () => {
	let { errCode, data: [item = {}] = [] } = await merchantCloudObj.list({ locating: locationStore.location, pageSize: 1 });
	orderStore.merchantInfo = item;
});

const getAddress = async () => {
	try {
		let { errCode, data: [item = {}] = [] } = await addressCloudObj.list({ pageSize: 1 });
...
watch(
	() => ({ merchantInfo: orderStore.merchantInfo, addressInfo: orderStore.addressInfo }),
	(nv, ov) => {
	...
	<view class="page-wrap">
		<view class="address-info-wrap">
			<view class="tabs-wrap">
				<view class="tabs">
					<view class="tab-item" :class="{ active: deliveryType === 0 }" @click="deliveryType = 0">
						<text class="iconfont icon-Newuserzone"></text>
						<text class="text">自提</text>
					</view>
					<view class="tab-item" :class="{ active: deliveryType === 1 }" @click="deliveryType = 1">
			...
			<view class="content-wrap">
				<view class="store-wrap">
					<view class="top-wrap" @click="routerTo('/pages/merchant/store?type=select')">
						<view class="left-wrap">
							<view class="label">供货商家：</view>
							<view class="name">{{ orderStore.merchantInfo?.store || '请选择' }}</view>
							<view class="address" v-if="orderStore.merchantInfo._id">
								{{ orderStore.merchantInfo?.address + orderStore.merchantInfo?.house }}
							</view>
						</view>
				...
				<view class="self-pick-box" v-if="deliveryType === 0">
					<view class="row-wrap pick-user-wrap">
						...
						<view class="value-wrap">
							<view class="text-wrap">
								<input type="number" placeholder="取货人联系电话" v-model.number="selfTakeInfo.phone" style="text-align: right" :maxlength="11" @blur="handleTakePhone" />
							</view>
							<uni-icons class="arrow" type="compose" color="#aaaaaa" size="20" />
						</view>
					</view>
					...
					<view class="row-wrap pick-time-wrap">
						...
						<view class="value-wrap">
							<view class="text-wrap">
								<uni-datetime-picker v-model="selfTakeInfo.time" :start="selfPickDateLimit[0]" :end="selfPickDateLimit[1]" :border="false" :clear-icon="false" :hide-second="true" />
							</view>
							<uni-icons class="arrow" type="right" color="#aaaaaa" size="30rpx" />
						</view>
					</view>
				</view>

				<view class="delivery-box" v-if="deliveryType === 1">
					<view class="row-wrap address-wrap" @click="routerTo('/pages/my/address?type=select')">
						<view class="left-wrap">
							<view class="label">收货信息：</view>
							<view class="name" v-if="orderStore.addressInfo?._id">{{ orderStore.addressInfo.name }}，{{ orderStore.addressInfo.phone }}</view>
							<view class="name" v-else style="font-weight: bold">请选择收货地址</view>
							<view class="address" v-if="orderStore.addressInfo?._id">{{ orderStore.addressInfo.address + orderStore.addressInfo.house }}</view>
						</view>
						<view class="right-wrap">
							<uni-icons class="arrow" type="right" size="30rpx" color="#aaa" />
						</view>
					...
					<view class="row-wrap pick-user-wrap">
						...
						<view class="value-wrap" @click="handleTransport">
							<view class="text-wrap" v-if="transportList[transportType]?.name">{{ transportList[transportType].name }}费￥{{ formatPrice(transportList[transportType]?.fee) }}</view>
							<uni-icons class="arrow" type="right" color="#aaaaaa" size="30rpx" />
						</view>
					</view>
				</view>
		...
		<view class="goods-list-wrap">
			<view class="goods-item" v-for="(item, index) in orderStore.orderGoodsList" :key="item._id">
			...
			<view class="type-list">
				<view class="type-item" @click.stop="payType = 0">
					<view class="label-wrap">
						<radio :checked="payType === 0" activeBackgroundColor="#EF5350" style="transform: scale(0.6)" />
				...
				<view class="type-item" @click.stop="payType = 1">
					<view class="label-wrap">
						<radio :checked="payType === 1" activeBackgroundColor="#EF5350" style="transform: scale(0.6)" />
		...
		<view class="amount-wrap">
			<view class="head-wrap">
				...
				<view class="right-wrap">￥{{ formatPrice(orderStore.orderPriceTotal) }}</view>
			</view>
		...
		<uv-action-sheet ref="transportRef" :actions="transportList" title="配送方式" @select="transportSelect"></uv-action-sheet>

		<uni-pay :toSuccessPage="false" ref="payRef" @success="paySuccess" @cancel="payCancel" @fail="payFail" @create="payCreate"></uni-pay>
	</view>
</template>

...
let id;
const categoryCloudObj = uniCloud.importObject("admin-product-category",{customUI:true});
const goodsCloudObj = uniCloud.importObject("admin-product-goods",{customUI:true})

const formRef = ref(null);
const skuItem = ()=>({_id:uuid(12),name:"",price:null,market_price:null,weight:500,status:true})
const formData = ref({
	name:"",
	category_id:"",
	is_on_sale:true,
...
const onSubmit = async()=>{
	try{
		...
		if(unref(formData).goods_banner_imgs.length){
			let uploadPromises = unref(formData).goods_banner_imgs.map((item,index)=>{
		...
		let {errCode} = unref(id) ? await goodsCloudObj.update({_id:unref(id),..._formData}) : await goodsCloudObj.add(unref(_formData));
				...
				<el-form 
				    ...
				    label-width="100">
						...
						<el-form-item label="缩略图">							
							<xxm-select-image :width="100" ratio="1/1" 
						...
						<el-form-item label="商品规格" prop="sku">
							<el-table :data="formData.sku" stripe border>
								<el-table-column label="规格名称" width="220">
								...
								<el-table-column label="现价">
									<template #default="scope">
										<el-input-number :min="0.01" v-model="scope.row.price" placeholder="现价" :precision="2">
											<template #prefix>
											  <text>￥</text>
										    </template>
										</el-input-number>
								...
								<el-table-column label="原价">
									<template #default="scope">
										<el-input-number :min="0.01" v-model="scope.row.market_price" placeholder="原价" :precision="2">
											<template #prefix>
											  <text>￥</text>
										    </template>
										</el-input-number>
								...
								<el-table-column label="重量">
									<template #default="scope">
										<el-input-number :min="1" v-model="scope.row.weight" placeholder="原价">
											<template #suffix>
											  <text>克</text>
										    </template>
										</el-input-number>
								...
								<el-table-column label="操作" width="100">
									<template #default="scope">
										<view class="skuBtn">
											...
											@click="delSku(scope.$index)" type="danger" :icon="Delete" circle />
											<el-button 
											v-if="scope.$index == formData.sku.length-1 && formData.sku.length<9" 
											@click="addSku" type="success" :icon="Plus" circle />
										</view>
									</template>
								</el-table-column>

...
import { routerTo, showLoading, showModal, showToast, getMerchantState } from '@/utils/common.js';
...
const previewImg = (index) => {
	if (!unref(tableData)[index].pic) return;
	uni.previewImage({
		urls: [unref(tableData)[index].pic]
	});
};
...
const delTable = async (id) => {
	try {
		...
		let { errCode } = await merchantCloudObj.remove({ _id: id });
		...
		<view class="uni-container">
			<el-table v-loading="loading" :data="tableData" stripe border style="width: 100%">
				...
				<el-table-column label="审核状态">
					<template #default="scope">
						<view>
							<el-tag size="small" :type="getMerchantState(scope.row.state).tagType">
								{{ getMerchantState(scope.row.state).label }}
							</el-tag>
						</view>
					</template>
				</el-table-column>

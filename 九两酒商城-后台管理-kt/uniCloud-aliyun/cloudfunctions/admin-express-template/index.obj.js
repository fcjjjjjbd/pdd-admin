...
module.exports = {
	...
	async add(params = {}) {
		try {
			let { errCode, errMsg, id } = await dbJQL.collection('JLJ-express-template').add({ ...params });
			if (errCode !== 0) return result({ errCode: 400, errMsg: 'error', custom: errMsg });
			if (params.is_default) {
				dbJQL.collection('JLJ-express-template').where(`_id != '${id}'`).update({ is_default: false });
			}
	...
	async update(params = {}) {
		try {
			...
			if (rest.is_default) {
				dbJQL.collection('JLJ-express-template').where(`_id != '${_id}'`).update({ is_default: false });
			}
	...
	async remove({ _id } = {}) {
		try {
			...
			let { errCode, deleted, errMsg } = await dbJQL.collection('JLJ-express-template').where(`_id == '${_id}'`).remove();
			if (errCode !== 0) return result({ errCode: 400, errMsg: 'error', custom: errMsg });
	...
	async detail({ _id } = {}) {
		try {
			if (!_id) return result({ errCode: 400, errMsg: 'error', custom: '_id不能为空' });
			let { errCode, data } = await dbJQL.collection('JLJ-express-template').doc(_id).field(`_id,name,is_default,rules,remote`).get({ getOne: true });

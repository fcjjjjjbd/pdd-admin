	...
	async list({ pageSize = 10, pageCurrent = 1, keyword = '', status } = {}) {
		try {
			...
			let {
				...
				.field(
					`order_no,total_fee,create_date,delivery_type,transport_info.type as transport_type,order_status,payment_method,arrayElemAt(user_id.nickname,0) as nickname,arrayElemAt(user_id.avatar,0) as avatar,goods_price`
				)
				.get({ getCount: true });
	...
	async detail({ _id } = {}) {
		try {
			...
			let { errCode, data } = await dbJQL
				.collection(orderTemp, userTemp)
				.field(
					`order_no,total_fee,create_date,delivery_type,transport_info.type as transport_type,order_status,payment_method,arrayElemAt(user_id.nickname,0) as nickname,arrayElemAt(user_id.avatar,0) as avatar,goods_price,goods_list`
				)
				.get({ getOne: true });

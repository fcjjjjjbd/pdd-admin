...
module.exports = {
	...
	async add(params = {}) {
		try {
			if (!(Array.isArray(params.location) && params.location.length === 2)) return result({ errCode: 400, errMsg: 'required', custom: '没有传递用户经纬度location' });
			//地理位置转换
			let location = {
				type: 'Point',
			...
			let { errCode, errMsg, id } = await dbJQL.collection('JLJ-user-address').add({ ...params, location });
			if (errCode !== 0) return result({ errCode: 400, errMsg: 'error', custom: errMsg });
			if (params.is_default) {
				dbJQL.collection('JLJ-user-address').where(`_id != '${id}' && user_id == $cloudEnv_uid`).update({ is_default: false });
			}
	...
	async update(params = {}) {
		try {
			let { _id, ...rest } = params;
			if (rest.location) {
				if (!(Array.isArray(rest.location) && rest.location.length === 2)) return result({ errCode: 400, errMsg: 'required', custom: '没有传递经纬度location' });
				let locationObj = {
					type: 'Point',
			...
			if (rest.is_default) {
				dbJQL.collection('JLJ-user-address').where(`_id != '${_id}' && user_id == $cloudEnv_uid`).update({ is_default: false });
			}
	...
	async remove({ _id } = {}) {
		try {
			if (!_id) return result({ errCode: 400, errMsg: 'error', custom: '_id不能为空' });
			let { errCode, deleted, errMsg } = await dbJQL.collection('JLJ-user-address').where(`_id == '${_id}' && user_id == $cloudEnv_uid`).remove();
			if (errCode !== 0) return result({ errCode: 400, errMsg: 'error', custom: errMsg });

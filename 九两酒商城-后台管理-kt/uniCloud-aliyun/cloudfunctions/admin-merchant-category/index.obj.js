...
module.exports = {
	...
	async add(params = {}){
		try{
			let {id,errCode,errMsg} = await dbJQL.collection("JLJ-merchant-category").add(params);
	...
	async list(){
		try{
			let {errCode,data} = await dbJQL.collection("JLJ-merchant-category")
	...
	async update({_id,...rest}={}){
		try{
			...
			let {errCode,errMsg,updated} = await dbJQL.collection("JLJ-merchant-category").doc(_id).update({...rest,last_modify_date:Date.now()});
			if(errCode!==0) return result({errCode:400,errMsg:"error",custom:errMsg});
	...
	async remove({_id}={}){
		try{
			...
			let {errCode,errMsg,deleted} = await dbJQL.collection("JLJ-merchant-category").doc(_id).remove();
			if(errCode!==0) return result({errCode:400,errMsg:"error",custom:errMsg});
	...
	async detail({_id}={}){
		try{
			if(!_id) return result({errCode:400,errMsg:"error",custom:"_id不能为空"});
			let {errCode,data} = await dbJQL.collection("JLJ-merchant-category").doc(_id)

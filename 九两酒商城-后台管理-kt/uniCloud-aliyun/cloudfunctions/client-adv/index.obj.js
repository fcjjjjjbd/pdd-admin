...
module.exports = {
  ...
  async categorylist({
    ...
    try {
      ...
      let { data, errCode } = await dbJQL
        .collection("pdd-adv")
        .where(`category_id=='${category_id}' && order_status != 0`)
  ...
  async detailxg(id = "") {
    try {
      ...
      let { data, errCode } = await dbJQL.collection("pdd-adv").doc(id).get({
        getOne: true,
      });
      if (errCode !== 0)
        return result({

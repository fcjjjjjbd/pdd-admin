...
module.exports = {
  ...
  async add(params = {}) {
    try {
      if (!params.sku.every((item) => item.name && item.price && item._id))
        return result({
          errCode: 400,
          errMsg: "error",
          custom: "sku内name、price、_id必填",
        });
  ...
  async update({
    ...
    try {
      ...
      if (rest.sku) {
        ...
        if (!rest.sku.every((item) => item.name && item.price && item._id))
          return result({
            errCode: 400,
            errMsg: "error",
            custom: "sku内name、price、_id必填",

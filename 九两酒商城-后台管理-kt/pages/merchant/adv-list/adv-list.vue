  ...
  const getData = async () => {
    try {
      ...
      let {
        ...
        data,
        total
      } = await goodsCloudObj.advlist(query.value);
      console.log(data)
    ...
    <view class="uni-header">
      ...
      <view class="uni-group">
        ...
        <button class="uni-button" type="primary" size="mini"
          @click="routerTo('./edit')">{{$t('common.button.add')}}</button>
      </view>
    </view>
    ...
    <view class="uni-container">
      <el-table v-loading="loading" :data="tableData" stripe border style="width: 100%">
        ...
        <el-table-column label="是否上架">
          <template #default="scope">
            <div>
              <el-switch :model-value="scope.row.is_on_sale" @change="saleChange($event,scope.$index)" />
            </div>
          </template>
        </el-table-column>

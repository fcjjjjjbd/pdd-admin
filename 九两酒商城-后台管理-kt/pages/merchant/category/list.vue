...
const categoryCloudObj = uniCloud.importObject('admin-merchant-category', { customUI: true });
	...
	<view class="fix-top-window">
		<view class="uni-header">
			...
			<view class="uni-group">
				<button class="uni-button" type="primary" size="mini" @click="handleAdd">{{ $t('common.button.add') }}</button>
			</view>
		</view>

		<view class="uni-container">
			<el-table v-loading="loading" :data="tableData" stripe border style="width: 100%">
				<el-table-column prop="name" show-overflow-tooltip label="分类名称" width="200" />
				<el-table-column label="排序">
					<template #default="scope">
						<el-input-number :model-value="scope.row.sort" @change="sortChange($event, scope.$index)">
				...
				<el-table-column label="更新日期">
					<template #default="scope">
						<div>
							{{ dayjs(scope.row.last_modify_date).format('YYYY-MM-DD HH:mm:ss') }}
						</div>
					</template>
				</el-table-column>
				...
				<el-table-column label="操作" width="180">
					<template #default="scope">
						<div>
							<el-button type="primary" @click="onUpdate(scope.$index)">{{ $t('common.button.edit') }}</el-button>
							<el-button type="danger" @click="onRemove(scope.$index)">{{ $t('common.button.delete') }}</el-button>
						</div>
					</template>
				</el-table-column>
			</el-table>
		...
		<el-dialog :title="`${typeName}分类`" width="800" v-model="editDialog" destroy-on-close :close-on-click-modal="false" :z-index="999">
			<EditCategory

...
const sortChange = async (e, index) => {
  try {
    ...
    loading.value = true;
    let { errCode, data } = await categoryCloudObj.twoupdate({ _id, sort: e });
...
const onRemove = async (index) => {
  try {
    ...
    let _id = unref(tableData)[index]._id;
    loading.value = true;
    let { errCode, data } = await categoryCloudObj.tworemove({ _id });
...
const getData = async () => {
  try {
    loading.value = true;
    let { errCode, data } = await categoryCloudObj.twolist();

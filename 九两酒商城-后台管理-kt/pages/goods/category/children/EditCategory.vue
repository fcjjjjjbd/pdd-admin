...
const onSubmit = async()=>{
	try{
		...
		loading.value = true;
		let {errCode,data,errMsg} = unref(formData)._id ? await categoryCloudObj.update(unref(formData)) : await categoryCloudObj.add(unref(formData));
	...
	<view class="layout">
		<el-form ref="formRef" label-width="100px" :model="formData" :rules="rules">
			...
			<el-form-item label="分类描述">
				<el-input v-model="formData.description" placeholder="请输入分类描述" clearable></el-input>
			</el-form-item>
		...
		<view class="dialog-footer">
			...
			<el-button type="primary" @click="onSubmit" :loading="loading">确认{{typeName}}</el-button>
		</view>
	</view>
</template>

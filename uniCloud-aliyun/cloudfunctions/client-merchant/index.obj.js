...
module.exports = {
		...
		try {
			...
			if (location.length === 2 && !keyword && !code) {
				...
				var { data, errCode, count } = await dbJQL
					...
					.field(`_id,store,address,house,location,pics`)
					.skip(pageCurrent)
					.limit(pageSize)
					.get({ getCount: true });
				...
				*/
				...
				var { data, errCode, count } = await dbJQL
					.collection('JLJ-merchant-list')
					.where(wre)
					.field(`_id,store,address,house,location,arrayElemAt(pics,0) as pic`)
	...
	async item({ _id } = {}) {
		try {
			...
			let { errCode, data } = await dbJQL
				.collection(listTemp, categoryTemp)
				.field(`_id, phone, store, arrayElemAt(category_id.name,0) as category_name, address,house,video,pics`)
				.get({ getOne: true });
